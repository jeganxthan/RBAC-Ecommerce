          ...
          {(!productData || productData.length === 0) ? (
            ...
            productData.map((product) => (
              <div key={product._id} className="flex flex-col items-center card hover:scale-95" onClick={()=>navigate(`/seller/product/${product._id}`)}>
                ...
                {isModalOpen && editingProduct?._id === product._id && (
                  <EditProductModal
...
<div className='flex flex-row justify-between'>
                ...
                <button onClick={() => openEditModal(product)} className="mr-2 bg-blue-700 rounded-lg p-2 text-white hover:bg-blue-500">
                  Edit
                </button>
                <button onClick={() => deleteProduct(product._id)} className='mr-2 bg-red-700 rounded-lg p-2 text-white hover:bg-red-500'>
                  Delete
                </button>
</div>
              </div>
